run_model <- function(effort, gear.mgmt, nsc, nspecies, t, Lmat, M1, phi, L.lower, L.upper, W.a, W.b, q, alpha, beta, suit, ration, other, weight, sc_Linf, phi.min){
     #' run_model
     #' 
     ...
     # N.ijte 
     # 
     # x.test2 <- array(NA, dim = c(2,2,3,5))
     # for(i in 1:5){
     #         x.test2tmp <- array(as.array(x.test)[i, ], dim = c(2, 2, 3))
     #         x.test2[, , , i] <- x.test2tmp 
     # }
